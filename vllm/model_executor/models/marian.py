...
# Adapted from https://github.com/huggingface/transformers/blob/v4.45.2/src/transformers/models/marian/modeling_marian.py
# by Rachel Wicks (rewicks@jhu.edu)
...
class MarianParallelLMHead(ParallelLMHead):
    """
...
class MarianEncoderLayer(nn.Module):
    def __init__(
        ...
        self.act = get_act_fn("relu", quant_config, ffn_intermediate_size)
...
class MarianDecoderLayer(nn.Module):
    def __init__(
        ...
        RACHEL: The following comment is from the vLLM port of the BART codebase.
        RACHEL: I think the Marian huggingface port is based off the BART model, so I have included this as well.
...
class MarianEncoder(nn.Module):
    ...
    self attention layers. Each layer is a [`MarianEncoderLayer`].
    Args:
        config: MarianConfig
    ...
    def __init__(self,
        ...
        self.layers = nn.ModuleList([MarianEncoderLayer(config,cache_config,quant_config) for _ in range(config.encoder_layers)])

        # RACHEL: there is supposedly no layer-norm here even though the BART model has some. I will assume this is correct.
    ...
    def forward(self, input_ids: torch.Tensor, positions: torch.Tensor,
        ...
        # RACHEL : again, BART has some layernorm here
...
class MarianDecoder(nn.Module):
    ...
    Each layer is a [`MarianDecoderLayer`]
    Args:
        config: MarianConfig
        embed_tokens (nn.Embedding): output embedding
    """
    ...
    def __init__(
        ...
        self.layers = nn.ModuleList([MarianDecoderLayer(config,cache_config,quant_config) for _ in range(config.decoder_layers)])

        # RACHEL: more potential layernorm here...
...
class MarianForConditionalGeneration(nn.Module):
    ...
    def __init__(self,
        ...
        self.unpadded_vocab_size = config.vocab_size if config.share_encoder_decoder_embeddings else config.decoder_vocab_size
        ...
        self.final_logits_bias = nn.Parameter(data=torch.zeros(1, config.vocab_size))
        # self.register_buffer("final_logits_bias", torch.zeros((1, config.vocab_size)))
        self.lm_head = MarianParallelLMHead(config.vocab_size,
                                          config.d_model,
    ...
    def compute_logits(
        ...
        logits = self.logits_processor(self.lm_head, hidden_states,
                                       sampling_metadata, embedding_bias = self.final_logits_bias)
        return logits
